...
$currency = $viewData['currency'];//var_dump($currency->_priceConfig['basePrice'][0]);die;
?>
<div class="product-price">
	...
	if ($product->prices['salesPrice']<=0 and VmConfig::get ('askprice', 1) and isset($product->images[0]) and !$product->images[0]->file_is_downloadable) {
		...
		?>
		<div class="callforprice bold" href="#" rel="nofollow" ><?php echo vmText::_ ('COM_VIRTUEMART_PRODUCT_ASKPRICE') ?></div>
		<?php
	} 
	else {

		// if($product->prices['product_override_price']>0 and $product->prices['override']=='1')
		// 	echo '<span class="line-through">'.$currency->createPriceDiv ('basePrice', 'COM_VIRTUEMART_PRODUCT_BASEPRICE', $product->prices).'</span>';
		// else
