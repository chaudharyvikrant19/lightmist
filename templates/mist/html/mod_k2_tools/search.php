...
<div id="k2ModuleBox<?php echo $module->id; ?>" class="k2SearchBlock<?php if($params->get('moduleclass_sfx')) echo ' '.$params->get('moduleclass_sfx'); if($params->get('liveSearch')) echo ' k2LiveSearchBlock'; ?>">
	...
	<form action="<?php echo $action; ?>" method="get" autocomplete="off" class="searh-holder">

		<input type="text" placeholder="<?php echo JText::_('TPL_BISS_SEARCH_TEXT');?>" name="searchword" maxlength="<?php echo $maxlength; ?>" size="<?php echo $width; ?>" />
		...
		<?php else: ?>
		<button type="submit" class="biss-btn biss-btn-primary" id="submit_btn" onclick="this.form.searchword.focus();"><i class="fa fa-search"></i> </button>
		<?php endif; ?>
		<?php endif; ?>
