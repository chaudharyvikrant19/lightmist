...
if($stepsModel->getActiveStep() == 'database')
{
	...
	if(!empty($nextStep['step']))
	{
		...
		ADocument::getInstance()->appendButton(
			'GENERIC_BTN_SKIP', $url, 'warning', 'white forward', 'btnNext'
		);
	}
...
elseif($stepsModel->getActiveStep() == 'offsitedirs')
{
    ...
    $key = $stepsModel->getActiveSubstep();
    ADocument::getInstance()->appendButton(
        'GENERIC_BTN_NEXT', 'javascript:offsitedirsRunRestoration(\''.$key.'\'); return false;', 'primary', 'white arrow-right', 'btnPrev'
    );
}
elseif($stepsModel->getActiveStep() == 'setup')
{
    ...
    if($key)
    {
        ...
        ADocument::getInstance()->appendButton(
            'GENERIC_BTN_NEXT', 'javascript:setupRunRestoration(\''.$key.'\'); return false;', 'primary multisite', 'white arrow-right', 'btnPrev'
        );
    }
    else
    {
