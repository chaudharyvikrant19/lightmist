...
class AzuraPagebuilderControllerPageTemplate extends JControllerLegacy
{
	...
	private static function parseElement($element){
		...
		if($element->type === 'AzuraRow'||$element->type === 'AzuraRowInner'){
		...
		}elseif($element->type === 'AzuraColumn'||$element->type === 'AzuraColumnInner'){
	...
	private static function parseRowElement($element){
		$html = array();
		if($element->type == 'AzuraRowInner') : 
		$html[] = '<div class="azura-element-block azura-element row-element isrowinner" data-typeName="'.$element->type.'">';
		else :
		$html[] = '<div class="azura-element-block row-element pagebuilder-section isrow" data-typeName="'.$element->type.'">';
		endif;
		//$html[] = '<div class="azura-element-block row-element pagebuilder-section" data-typeName="'.$element->type.'">';
			...
			if(count($element->children)) {
				...
				if($element->type == 'AzuraRowInner') : 
				$html[] = '<div class="column-parent col-md-12"  data-typeName="AzuraColumnInner">';
				else :
				$html[] = '<div class="column-parent col-md-12"  data-typeName="AzuraColumn">';
					...
					$html[] = '</div>';
				if($element->type == 'AzuraRowInner') : 
					$html[] = '<div class="azura-element-settings-saved saved" data="'.rawurlencode('{"type":"AzuraColumnInner","id": "0","published":"1","language":"*", "content":"","attrs":{"columnwidthclass":"col-md-12"}}').'"></div>';
				else:
