...
?>
<?php if($element->type === 'AzuraColumnInner') :?>
<div class="column-parent <?php echo (!empty($element->attrs->columnwidthclass)? str_replace("col-md-", "col-sm-", $element->attrs->columnwidthclass) : 'col-sm-12');?><?php echo (isset($element->attrs->smoffsetclass)? ' '.$element->attrs->smoffsetclass : ' col-sm-offset-0');?><?php echo (isset($element->attrs->mdwidthclass)? ' '.$element->attrs->mdwidthclass : '');?><?php echo (isset($element->attrs->mdoffsetclass)? ' '.$element->attrs->mdoffsetclass : '');?><?php echo (isset($element->attrs->lgwidthclass)? ' '.$element->attrs->lgwidthclass : '');?><?php echo (isset($element->attrs->lgoffsetclass)? ' '.$element->attrs->lgoffsetclass : '');?>"  data-typeName="AzuraColumnInner">
<?php else :?>
...
<?php endif;?>
	...
	<div class="column-wrapper">
		...
		<?php if($element->type === 'AzuraColumn') :?>
