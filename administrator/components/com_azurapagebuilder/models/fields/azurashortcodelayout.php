...
class JFormFieldAzuraShortcodelayout extends JFormField
{
	...
	protected function getInput()
	{
		...
		if ($shortcode && $client)
		{
			...
			// Build the search paths for shortcode layouts.
			$plugin_shortcode_path = JPath::clean($client->path . '/plugins/azura/azuraoption/shortcodes/templates');

			// Prepare array of shortcode layouts
			$shortcode_layouts = array();

			$plugin_shortcode_layouts = array();
			...
			// Add the layout options from the plugin shortcode path.
			if (is_dir($plugin_shortcode_path) && ($plugin_shortcode_layouts = JFolder::files($plugin_shortcode_path, '^'.$shortcode.'([-]+[^_]*\.php|\.php)$')))
			{
				// if(count($template_layouts)){
				// 	foreach ($plugin_shortcode_layouts as $i => $file)
				// 	{
				// 		// Remove layout that already exist in shortcode ones
				// 		if (in_array($file, $template_layouts))
				// 		{
				// 			unset($plugin_shortcode_layouts[$i]);
				// 		}
				// 	}

				// }
				if (count($plugin_shortcode_layouts))
				{

					// Create the group for the shortcode
					$groups['azuraoption'] = array();
					$groups['azuraoption']['id'] = $this->id . '_azuraoption';
					$groups['azuraoption']['text'] = JText::sprintf('--From AzuraOption Plugin --', $shortcode);
					$groups['azuraoption']['items'] = array();

					foreach ($plugin_shortcode_layouts as $file)
					{
						// Add an option to the shortcode group
						$value = basename($file, '.php');
						$text = $value;
						$groups['azuraoption']['items'][] = JHtml::_('select.option', 'azuraoption:' . $value, $text);
					}
				}
			}
