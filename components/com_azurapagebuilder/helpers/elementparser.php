...
class AzuraShortcode {
    ...
    public static function buildStyle($atts){
    ...
    public static function buildResponsive($atts,$prefix_cl = 'azp_'){
        //$return = array();
        $responsiveTxt = self::parseResponsive(cth_shortcode_atts(array(
    ...
    public static function buildAnimation($atts,$trigger_cl = 'animate-in'){
        $return = array();
...
class ElementParser
{
    ...
    public static function addShortcodeFiles()
    {
        ...
        if( file_exists( JPATH_PLUGINS .'/azura/azuraoption/shortcodes/azp-override.php' ) && !is_dir( JPATH_PLUGINS .'/azura/azuraoption/shortcodes/azp-override.php' ) ){
            require_once JPATH_PLUGINS .'/azura/azuraoption/shortcodes/azp-override.php';
        }
    ...
    private static function do_shortcode_tag( $m ) {
        ...
        $c_name = 'AzuraShortcode_'.strtolower($tag);
        if ( isset( $m[5] ) ) {
            // enclosing tag - extra parameter
            if(class_exists($c_name)){
                return $m[1] . call_user_func( array($c_name,'content'), $attr, $m[5], $tag ) . $m[6];
            }else{
                if(function_exists(self::$shortcode_tags[$tag])){
            ...
            // self-closing tag
            if(class_exists($c_name)){
                return $m[1] . call_user_func( array($c_name,'content'), $attr, null,  $tag ) . $m[6];
            }else{
                if(function_exists(self::$shortcode_tags[$tag])){
